 ...
 *
 */
template<typename Incrementable> class run_iterator : public boost::iterator_facade<
...
public:
     ...
     * @return int
     */
    int runElementCount(std::vector<Incrementable> const& intvec, int &run_counter, int &min_run, int& max_run);
...
private:
     ...
     */
    Incrementable const& dereference() const
    {
    ...
    {
        // Note -- we are not comparing against the currentElement
        //	-- if we decide to add this, we need to also set currentElement to 0 after it reaches the end()
        //  -- that is -- the beginning of updateElement() should be
        //  -- 		if( runIter == runIterEnd)	{ currentElement = 0;		return;}
...
template<typename Incrementable> int run_iterator<Incrementable>::runElementCount(std::vector<Incrementable> const& intvec, int &run_counter, int &min_run, int &max_run)
{
    int counter = 0;
    for(typename run_iterator<Incrementable>::RunContainerCIt it = intvec.begin(); it != intvec.end(); ++it)
    {
        if(*it < 0)
        {
            int current_run = (1 + *(++it));
            if(min_run > current_run)
                min_run = current_run;
